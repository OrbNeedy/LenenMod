...
namespace lenen.Content.Projectiles
{
    public class JudgementCut : ModProjectile
    {
        ...
        {
            return base.CanHitPlayer(target) && Projectile.scale > 0.75f;
        }
        ...
        {
            if (Projectile.scale <= 0.75f) return false;

            Vector2 end = new Vector2(0, 800).RotatedBy(Projectile.rotation);
            Vector2 start = -end;
            float w = 4;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            Main.EntitySpriteDraw(
                BulletUtils.GetTexture(BulletColor, (int)Sheet.Pellet) with
                {
                    ...
                    scale = new Vector2(Projectile.scale, 162)
                }
                );
