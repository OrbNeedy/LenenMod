...
{
    public class KuroLaser : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.type != ModContent.ProjectileType<KuroLaser>() || !hostPrism.active ||
                hostPrism.type != ModContent.ProjectileType<KuroLaserHoldout>())
            {
                Projectile.Kill();
                return;
            }
            ...
            float additionalOffsetRotation = hostPrism.Center.DirectionTo(Main.MouseWorld).ToRotation();
            Vector2 beamIdOffset = new Vector2(0, -50).RotatedBy((BeamID * MathHelper.PiOver4) + 
            ...
            Projectile.velocity = hostPrismDir.RotatedBy(beamRotation.ToRotation());
        ...
        {
            Utils.LaserLineFraming lineFraming = new Utils.LaserLineFraming(KuroLaserDraw);

            DelegateMethods.c_1 = beamColor;
            Utils.DrawLaser(spriteBatch, texture, startPosition, endPosition, drawScale, lineFraming);
        }

        private static void KuroLaserDraw(int stage, Vector2 currentPosition, float distanceLeft, Rectangle lastFrame, out float distCovered, out Rectangle frame, out Vector2 origin, out Color color)
        {
            color = Color.White;
            switch (stage)
            {
